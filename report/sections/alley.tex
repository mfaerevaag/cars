...
When a car is leaving we follow the same pattern of steps, only
in the opposite order. The car starts with decrementing the
\code{count} variable and then notifies the waiting car(s) by calling either \code{notify()} or \code{notifyAll()}.

\code{notify()} is used when the last of the upwards bound cars leave, as there is only a single line of downwards bound cars waiting, whereas there are two lines of cars waiting to go upwards and therefore \code{notifyAll()} is called when the last car headed downwards leaves.
